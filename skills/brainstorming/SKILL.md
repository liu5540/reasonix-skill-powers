...
> **硬性审计门槛**：在你展示设计方案并获得用户批准之前，不要调用任何实现技能、编写任何代码、搭建任何项目或采取任何实现行动。这适用于所有项目，无论看起来多简单。
...
5. **编写设计文档** — 保存到 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` 并 commit
6. **规格自检** — 快速内联检查占位符、矛盾、模糊性、范围（详见下方）
7. **用户审查书面规格** — 在继续之前请用户审查规格文件
8. **过渡到实现** — 调用 writing-plans 创建实现计划
...
**终止状态是调用 writing-plans。** 头脑风暴之后你唯一要调用的技能是 writing-plans。
